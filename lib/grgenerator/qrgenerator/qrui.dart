...
//                   onPressed: () => _saveQrAsPdf(context, state.encryptedData),
//                 ),
//                 IconButton(
...
//                           child: QrImageView(
//                             data: state.encryptedData, // Show the generated QR Code
//                             size: 250,
...
//  Future<void> _saveQrAsPdf(BuildContext context, String encryptedData) async{
...
class QrPage extends StatelessWidget {
  ...
  final ScreenshotController screenshotController =
      ScreenshotController(); // For QR screenshot
              ...
              IconButton(
                ...
                onPressed:
                    state.encryptedData.isNotEmpty
                        ? () => _saveQrAsPdf(context, state.encryptedData)
                        : null, // Disable button if no QR
              ),
              IconButton(
                icon: Icon(Icons.share),
                onPressed:
                    state.encryptedData.isNotEmpty
                        ...
                        : null, // Disable if no QR
              ),
            ],
          ),
          body: Center(
            child:
                state.encryptedData.isEmpty
                    ? const Text(
                      ...
                      children: [
                        Screenshot(
                          ...
                          child: QrImageView(
                            data:
                                state
                                    .encryptedData, // Show the generated QR Code
                            size: 250,
                            backgroundColor: Colors.white,
                          ),
                        ),
  ...
  Future<void> _saveQrAsPdf(BuildContext context, String encryptedData) async {
    ...
    final savedPath =
        "${downloadsDir!.path}/QRCode_${formattedDate}_${now.hour}-${now.minute}-${now.second}.pdf";
    final file = File(savedPath);
    await file.writeAsBytes(await pdf.save());
